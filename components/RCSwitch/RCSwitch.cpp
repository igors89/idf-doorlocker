...
void enableTransmit(RCSWITCH_t * RCSwitch, int nTransmitterPin) {
	...
	gpio_reset_pin(static_cast<gpio_num_t>(RCSwitch->nTransmitterPin) );
	gpio_set_direction( static_cast<gpio_num_t>(RCSwitch->nTransmitterPin), GPIO_MODE_OUTPUT );
}
...
void sendCode(RCSWITCH_t * RCSwitch, unsigned long code, unsigned int length) {
	...
	gpio_set_level( static_cast<gpio_num_t>(RCSwitch->nTransmitterPin), LOW );
...
#endif
	gpio_set_level(static_cast<gpio_num_t>(RCSwitch->nTransmitterPin), firstLogicLevel );
	//ets_delay_us(RCSwitch->protocol.pulseLength * pulses.high);
	esp_rom_delay_us(RCSwitch->protocol.pulseLength * pulses.high);
	gpio_set_level(static_cast<gpio_num_t>(RCSwitch->nTransmitterPin), secondLogicLevel);
...
esp_err_t enableReceiveInternal(RCSWITCH_t * RCSwitch) {
	...
	err = gpio_isr_handler_add(static_cast<gpio_num_t>(RCSwitch->nTransmitterPin), handleInterrupt, RCSwitch);
...
void disableReceive(RCSWITCH_t * RCSwitch) {
	...
	gpio_isr_handler_remove(static_cast<gpio_num_t>(RCSwitch->nTransmitterPin)); // nReceiverInterrupt is GPIO
	RCSwitch->nReceiverInterrupt = -1;
}
